...
public class AppRunner implements ApplicationRunner {
    ...
    private void createProjectTestData(int index) {
        ...
        Project project = Project.builder().content("내용들").title(title + index).endDate(LocalDateTime.now()).
                startDate(LocalDateTime.now()).deadline(LocalDateTime.now()).location(location[random.nextInt(location.length)]).
                createdDate(LocalDateTime.now()).status("모집").leader(user).developerRecruits(1).designerRecruits(0).plannerRecruits(0).
                marketerRecruits(1).etcRecruits(0).build();
